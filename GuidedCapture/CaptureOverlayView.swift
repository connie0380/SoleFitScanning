            ...
            HStack(alignment: .bottom, spacing: 0) {
                HStack(spacing: 0) {
                    if case .capturing = session.state {
                        PlayerView(url: URL(string: "https://codeskulptor-demos.commondatastorage.googleapis.com/GalaxyInvaders/pause.wav")!,
                                   isTransparent: false, isStacked: true, isInverted: false, shouldLoop: true)
                        .frame(width: 1, height: 1)
                        NumOfImagesButton(session: session)
                            .rotationEffect(rotationAngle)
                            .transition(.opacity)
                    } else if case .detecting = session.state {
                        PlayerView(url: URL(string: "https://codeskulptor-demos.commondatastorage.googleapis.com/GalaxyInvaders/theme_01.mp3")!, 
                                   isTransparent: false, isStacked: true, isInverted: false, shouldLoop: false)
                        .frame(width: 1, height: 1)
